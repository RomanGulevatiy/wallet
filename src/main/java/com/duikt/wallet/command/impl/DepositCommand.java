...
public class DepositCommand implements Command {
    ...
    public boolean canHandle(Update update) {
        return CommandUtils.isCommand(update, CommandName.DEPOSIT.getName());
    }
    ...
    public void handle(Update update) {
        ...
        if(parts.length < 3) {
            messageSender.send(chatId, "Please provide a user id and an amount. Usage: /deposit <id> <amount>");
            return;
        }
        ...
        try {
            ...
            messageSender.send(chatId, "Invalid format. Please provide a numeric user id and amount. Usage: /deposit <id> <amount>");
        }
        catch(UserNotFoundException ex) {
            messageSender.send(chatId, "User not found. Please check the id and try again.");
        }
        catch(WalletException ex) {
            messageSender.send(chatId, "Failed to deposit: " + ex.getMessage());
        }
    }

    @Override
    public String getCommand() {
        return CommandName.DEPOSIT.getName();
    }
}
